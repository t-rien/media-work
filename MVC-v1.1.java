...
class GamePanel extends JPanel implements KeyListener {
    ...
    private ImageIcon bicon, obs, road, wall, exwave, picon, item_number, item_power, micon, loss, win;           //変更->changed again by takezaki
	...
	micon = new ImageIcon("./View/monster-x.jpg");
	item_power = new ImageIcon("./View/walkr-x.jpg");
	item_number = new ImageIcon("./View/walkl-x.jpg");
	loss = new ImageIcon("./View/obs.jpg");
	win = new ImageIcon("./View/bomb.png");
	...
	for(int i=0; i<15; i++) {
	    for(int j=0; j<13; j++) {
            if(map[i][j].getStatus() == 1+8+128||map[i][j].getStatus() == 1+4+8||map[i][j].getStatus() == 1+4+8+16) {
                ...
                //ここに失敗の画面遷移のコードを
		g.drawImage(loss.getImage(), 0, 0, this);
            }else if(map[i][j].getStatus() == 128 + 64){                  //モンスターがアイテムに被った
                g.drawImage(micon.getImage(), i*75, j*75, this);
            }else if(map[i][j].getStatus() == 128 + 32){            //モンスターがアイテムに被った
                g.drawImage(micon.getImage(), i*75, j*75, this);
            }else if(map[i][j].getStatus() == 128 + 4 + 1){            //モンスターが一匹殺された
                g.drawImage(micon.getImage(), i*75, j*75, this);
		    ...
		    g.drawImage(win.getImage(), 0, 0, this);
            ...
            }else if(map[i][j].getStatus() == 128+1){             //モンスターが歩いている
                g.drawImage(micon.getImage(), i*75, j*75, this);
            }else if(map[i][j].getStatus() == 64 + 1) { // 爆弾を増やすアイテム
                g.drawImage(item_number.getImage(), i*75, j*75, this);
            } else if(map[i][j].getStatus() == 32 + 1) { //　パワーアップのアイテム
                g.drawImage(item_power.getImage(), i*75, j*75, this);
            } else if(map[i][j].getStatus() == 16 + 8 + 1) { // bomb + player + space
                ...
                g.drawImage(road.getImage(), i*75, j*75, this);
                g.drawImage(picon.getImage(), i*75, j*75, this);
            } else if(map[i][j].getStatus() == 4 + 2 + 1) { // melting obstacle
                //g.drawImage(picon.getImage(), i*75, j*75, this);
            } else if(map[i][j].getStatus() == 4 + 1) { // explosion + space
                g.drawImage(road.getImage(), i*75, j*75, this);
                g.drawImage(exwave.getImage(), i*75, j*75, this);
            } else if(map[i][j].getStatus() == 2 + 1) { // brakable wall + space
...
class Boomb extends TimerTask{                        // Viewでは ArrayListでBombを格納して処理する？
  ...
  public Boomb(Unit[][] map , Player player, GamePanel sp){            // 　プレイヤーの居るところに爆弾を置く
    ...
    if(flag  == 1)
      map[x][y].addStatus(4); // 爆弾が爆発うしたので、自体が余波になる
    else
      map[x][y].setStatus(1);
  }
