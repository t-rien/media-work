...
class SubPanel extends JPanel implements KeyListener {
    ...
    private ImageIcon picon,item_number, item_power;           //変更->changed again by takezaki
	...
	for(int i=0; i<15; i++) {
	    for(int j=0; j<13; j++) {
		if(map[i][j].getStatus() == 64 + 1) { // number up item
		    //g.drawImage(item_nuber.getImage(), i*75, j*75, this);
		} else if(map[i][j].getStatus() == 32 + 1) { // power up item
...
class Boomb extends TimerTask{                        // Viewでは ArrayListでBombを格納して処理する？
  ...
  public Boomb(Unit[][] map , Player player, SubPanel sp){            // 　プレイヤーの居るところに爆弾を置く
    ...
    for(int i=1;i <= range;i++){
        if(this.x+i < 15) {
            temp = map[this.x+i][this.y];
            if(temp.getStatus() == 16 + 1||temp.getStatus() == 0||(temp.getStatus() == 5 && cmd>0) ) {break; } // 爆弾や壁だったら、爆発を終了する
            if(temp.getStatus()+cmd >0)            if(temp.getStatus()+cmd >0 )
                temp.addStatus(cmd);
            if( temp.getStatus() == 3+4){             // 壊せる壁までしか、爆弾の余波が及ばない
                break;
            }else if(temp.getStatus() == 7-4){        // 壊せる壁までしか、爆弾の余波を戻しかつ空地にする
                temp.setStatus(1);
                break;
            }
        }
